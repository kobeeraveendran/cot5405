...
\begin{enumerate}
    ...
    are processing approximately half the array of the previous iteration (since the other half is discarded). Removing 
    unhelpful elements can be done in linear time as well, so at any given iteration we will do $\frac{n}{2^i}$ work to find the 
    $k^{th}$-smallest element, and another $\frac{n}{2^i}$ work to remove elements larger than it. Generalized over all iterations, we will do 
    $2n + n + \frac{n}{2} + \frac{n}{4} + \frac{n}{8} + \dots + \frac{n}{2^{i-1}} \approx 4n$ steps of total work, yielding an $O(n)$ total 
    time complexity. Depending on whether $n$ is a power of 2, and the subsequent amount of pruning work, the constant 
